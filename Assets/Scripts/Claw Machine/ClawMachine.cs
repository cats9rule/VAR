...
public class ClawMachine : MonoBehaviour
{
    ...
    private float startingDropChance = 0.5f;
    [SerializeField] private float dropIncrease = 0.05f;
    ...
    {
        while (isReturning && clawSocket.socketActive)
        {
            var random = Random.Range(0f, 1f); //proveriti generaciju
    ...
    IEnumerator GoBackState()
    {
        ...
        if (isDropped)
        {
            dropChance -= dropIncrease;
            isDropped = false;
        }
        else
        {
            dropChance = dropChance < startingDropChance ? startingDropChance : (dropChance + dropIncrease);
        }
